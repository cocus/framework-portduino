/*
  WiFiClientSecure.cpp - Client Secure class for targets using mbedtls
...
int WiFiClientSecure::connect(const char *host, uint16_t port, const char *CA_cert, const char *cert, const char *private_key) {
  IPAddress address;
  if (!IPAddress::hostByName(host, address)) {
    return 0;
  }
...
int WiFiClientSecure::setSocketOption(int level, int option, const void *value, size_t len) {
  ...
  if (res < 0) {
    ::printf("WiFiClientSecure fail on %d, errno: %d, \"%s\"\n", sslclient->socket, errno, strerror(errno));
  }
  return res;
}

int WiFiClientSecure::getSocketOption(int level, int option, const void *value, size_t size) {
  int res = getsockopt(sslclient->socket, level, option, (char *)value, (socklen_t *)&size);
  if (res < 0) {
    ::printf("WiFiClientSecure fail on fd %d, errno: %d, \"%s\"\n", sslclient->socket, errno, strerror(errno));
  }
  return res;
}
...
int WiFiClientSecure::getOption(int option, int *value) {
  ...
  int res = getsockopt(sslclient->socket, IPPROTO_TCP, option, (char *)value, &size);
  if (res < 0) {
    ::printf("WiFiClientSecure fail on fd %d, errno: %d, \"%s\"\n", sslclient->socket, errno, strerror(errno));
  }
  return res;
...
void WiFiClientSecure::setCACertBundle(const uint8_t *bundle, size_t size) {
  if (bundle != NULL && size > 0) {
    //esp_crt_bundle_set(bundle, size);
    ::printf("NOT IMPL setCACertBundle!");
