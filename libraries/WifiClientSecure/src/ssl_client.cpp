/* Provide SSL/TLS functions to any target using mbedtls.
*
* Adapted from the ssl_client1 example of mbedtls.
*
* Original Copyright (C) 2006-2015, ARM Limited, All Rights Reserved, Apache 2.0 License.
* Additions Copyright (C) 2017 Evandro Luis Copercini, Apache 2.0 License.
* Additions Copyright (c) 2026 Santiago Hormazabal.
*/
...
int start_ssl_client(
  ...
  int domain = AF_INET; // for now only ipv4 ip.type() == IPv6 ? AF_INET6 : AF_INET;
  log_d("Starting socket (domain %d)", domain);
  ssl_client->socket = -1;

  ssl_client->socket = socket(domain, SOCK_STREAM, IPPROTO_TCP);
  ...
  int res = ::connect(ssl_client->socket, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
...
#define ROE(x, msg)                                    \
  {                                                    \
    if (((x) < 0)) {                                   \
      log_e("posix Socket config of " msg " failed."); \
      return -1;                                       \
    }                                                  \
  }
  ROE(setsockopt(ssl_client->socket, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)), "SO_RCVTIMEO");
  ROE(setsockopt(ssl_client->socket, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)), "SO_SNDTIMEO");
  ...
  /*} else if (useRootCABundle) {
    if (ssl_client->bundle_attach_cb != NULL) {
      log_d("Attaching root CA cert bundle");
...
int send_ssl_data(sslclient_context *ssl_client, const uint8_t *data, size_t len) {
  ...
  while (sent < len) {
    ...
    if ((millis() - last_progress) > ssl_client->socket_timeout) {
      log_d("SSL write timed out.");
      return -1;
    }

    if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE && ret < 0) {
      log_d("Handling error %d", ret);
      return handle_error(ret);
    }
